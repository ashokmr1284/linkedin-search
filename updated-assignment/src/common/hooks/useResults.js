...
useEffect(() => {
  ...
  filteredResults = data.filter((item) => {
    ...
    let updateCompany = item.company.toUpperCase();
    let updateEducation = item.eyeColor.toUpperCase();
    let updateHobbies = item.favoriteFruit.toUpperCase();
    let updateNativePlace = item.gender.toUpperCase();
    let checkCase = (updateName.indexOf(checkSearchInput) !== -1) || (updateCompany.indexOf(checkSearchInput) !== -1) ||(updateEducation.indexOf(checkSearchInput) !== -1)  ||(updateHobbies.indexOf(checkSearchInput) !== -1)  ||(updateNativePlace.indexOf(checkSearchInput) !== -1) 
    return checkCase;
  });
